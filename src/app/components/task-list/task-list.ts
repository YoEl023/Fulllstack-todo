...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CreateTask, TabsModule, FormsModule, PaginationModule, TaskTable],
    ...
    this.taskService.getDeletedTasks().subscribe({
      next: (deleted) => {
        ...
        if (this.currentSearchTerm) {
          ...
          filtered = deleted.filter((task) => task.taskName.toLowerCase().includes(term));
        }

        const startIndex =
          (this.pagination.deleted.currentPage - 1) * this.pagination.deleted.itemsPerPage;
